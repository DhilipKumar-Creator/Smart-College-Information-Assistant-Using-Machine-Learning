// Smart College Information Assistant - ML Powered JavaScript
...
function extractIntent(userInput) {
    ...
    // Intent classification using keyword matching (simplified NLP)
    if (input.includes('recommend') || input.includes('suggest') || input.includes('best')) {
        return 'recommendation';
    } else if (input.includes('predict') || input.includes('forecast') || input.includes('estimate')) {
        return 'prediction';
    } else if (input.includes('route') || input.includes('navigation') || input.includes('path')) {
        return 'navigation';
    } else if (input.includes('analytics') || input.includes('dashboard') || input.includes('insights')) {
        return 'analytics';
    } else if (input.includes('where') || input.includes('location')) {
        return 'location';
    } else if (input.includes('faculty') || input.includes('professor') || input.includes('staff')) {
        return 'faculty';
...
function getMLRecommendations(query, department) {
    const dept = collegeData.departments.find(d => 
        d.code.toLowerCase() === department.toLowerCase()
    );
    
    if (!dept) return null;
    
    // Simulate ML-based faculty scoring
    const facultyScores = dept.faculty.map(faculty => ({
        ...faculty,
        mlScore: Math.random() * 100,
        expertise: ['AI/ML', 'Web Development', 'Data Science', 'Networking', 'Software Engineering'][Math.floor(Math.random() * 5)],
        rating: (Math.random() * 2 + 3).toFixed(1) // 3.0-5.0 rating
    }));
    
    // Sort by ML score
    facultyScores.sort((a, b) => b.mlScore - a.mlScore);
    
    return facultyScores.slice(0, 5); // Top 5 recommendations
}

// Predictive Analytics Functions
function predictAttendance(studentId, courseId) {
    ...
    const riskLevel = baseAttendance < 80 ? 'high' : baseAttendance < 90 ? 'medium' : 'low';
    
    return {
        predictedAttendance: Math.round(baseAttendance),
        trend: trend,
        riskLevel: riskLevel,
...
function predictPerformance(studentId, courseId) {
    ...
    const baseGrade = 70 + Math.random() * 25; // 70-95
    const grade = Math.round(baseGrade);
...
// Smart Navigation with ML
function getOptimalRoute(fromLocation, toLocation) {
    // Simulate ML-based route optimization
    const routes = [
        { path: 'Direct Route', distance: '200m', time: '3 min', congestion: 'low' },
        { path: 'Scenic Route', distance: '300m', time: '5 min', congestion: 'very low' },
        { path: 'Shortcut', distance: '150m', time: '2 min', congestion: 'medium' }
    ];
    
    // ML algorithm would consider: time, distance, congestion, weather, events
    const optimalRoute = routes[Math.floor(Math.random() * routes.length)];
    
    return {
        from: fromLocation,
        to: toLocation,
        recommended: optimalRoute,
        alternatives: routes.filter(r => r !== optimalRoute),
        mlFactors: ['Time optimization', 'Crowd prediction', 'Weather consideration']
    };
}
...
// Enhanced ML-Powered Response Handler
function processUserInput(input) {
    const lowerInput = input.toLowerCase().trim();
    const intent = extractIntent(input);
    
    // ML-powered recommendation queries
    if (intent === 'recommendation') {
        ...
        if (targetDept) {
            const recommendations = getMLRecommendations(input, targetDept);
            if (recommendations) {
                let html = `
                    <div class="ml-recommendation">
                        <h3>🤖 AI-Powered Faculty Recommendations</h3>
                        <p><strong>Department:</strong> ${targetDept.toUpperCase()}</p>
                        <p><strong>ML Algorithm:</strong> Collaborative Filtering + Content-Based</p>
                        <div class="recommendations-grid">
                `;
                
                recommendations.forEach((faculty, index) => {
                    html += `
                        <div class="recommendation-card">
                            ...
                            <div class="ml-metrics">
                                <span class="ml-score">ML Score: ${Math.round(faculty.mlScore)}</span>
                                <span class="rating">⭐ ${faculty.rating}</span>
                            </div>
                            <div class="expertise">${faculty.expertise}</div>
                        </div>
                    `;
                });
                ...
                html += `
                        ...
                        <p class="ml-note">💡 Recommendations based on student preferences, faculty expertise, and historical performance data.</p>
                    </div>
                `;
    ...
    if (intent === 'prediction') {
        if (lowerInput.includes('attendance')) {
            const prediction = predictAttendance('student123', 'course456');
            return `
                <div class="prediction-result">
                    <h3>🔮 Attendance Prediction</h3>
                    <div class="prediction-metrics">
                        <div class="metric">
                            <span class="metric-label">Predicted Attendance:</span>
                            <span class="metric-value ${prediction.riskLevel}">${prediction.predictedAttendance}%</span>
                        </div>
                        <div class="metric">
                        ...
                        <div class="metric">
                            ...
                            <span class="metric-value ${prediction.riskLevel}">${prediction.riskLevel}</span>
                        </div>
                        <div class="metric">
                    ...
                    <p class="ml-note">💡 Based on historical attendance patterns, course difficulty, and student behavior analysis.</p>
                </div>
            `;
        }
        
        if (lowerInput.includes('grade') || lowerInput.includes('performance')) {
            const prediction = predictPerformance('student123', 'course456');
            return `
                <div class="prediction-result">
                    <h3>🔮 Performance Prediction</h3>
                    <div class="prediction-metrics">
                        <div class="metric">
                            <span class="metric-label">Predicted Grade:</span>
                            <span class="metric-value">${prediction.predictedGrade}% (${prediction.letterGrade})</span>
                        </div>
                        <div class="metric">
                            <span class="metric-label">Confidence:</span>
                            <span class="metric-value">${prediction.confidence}%</span>
                    ...
                    <p class="ml-note">💡 Based on assignment scores, attendance, and historical performance patterns.</p>
                </div>
            `;
        }
    }
    
    // Smart navigation queries
    if (intent === 'navigation') {
        const classroomMatch = input.match(/([A-Z]{2}\d{3})/);
        if (classroomMatch) {
            const classroomCode = classroomMatch[1];
            const route = getOptimalRoute('Current Location', classroomCode);
            
            return `
                <div class="smart-navigation">
                    <h3>🧭 Smart Navigation to ${classroomCode}</h3>
                    <div class="route-info">
                        <div class="recommended-route">
                            <h4>🎯 Recommended Route</h4>
                            <p><strong>Path:</strong> ${route.recommended.path}</p>
                            <p><strong>Distance:</strong> ${route.recommended.distance}</p>
                            <p><strong>Time:</strong> ${route.recommended.time}</p>
                            <p><strong>Congestion:</strong> ${route.recommended.congestion}</p>
                        </div>
                        <div class="ml-factors">
                            <h4>🤖 ML Factors Considered:</h4>
                            <ul>
                                ${route.mlFactors.map(factor => `<li>${factor}</li>`).join('')}
                            </ul>
                        </div>
                    </div>
                </div>
    ...
    // Classroom location queries (enhanced)
    if (intent === 'location' || (lowerInput.includes('where is') || lowerInput.includes('location of'))) {
        const classroomMatch = input.match(/([A-Z]{2}\d{3})/);
        if (classroomMatch) {
            const classroomCode = classroomMatch[1];
...
function initializeCharts() {
    ...
    if (facultyCtx) {
        new Chart(facultyCtx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [45, 30, 20, 5],
                    backgroundColor: ['#48bb78', '#38a169', '#ed8936', '#e53e3e'],
                    borderWidth: 0
                }]
            },
            options: {
    ...
    if (departmentCtx) {
        new Chart(departmentCtx, {
            type: 'bar',
            data: {
                labels: ['CSE', 'AIDS', 'ECE', 'MECH', 'AE', 'IT'],
                datasets: [{
                    label: 'Performance Score',
                    data: [92, 89, 85, 82, 78, 75],
                    backgroundColor: [
